...
var Btc = function () {
  ...
  _createClass(Btc, [{
    ...
    value: function getTrustedInput(indexLookup, transaction) {
      ...
      var processInputs = function processInputs() {
        return (0, _utils.eachSeries)(inputs, function (input) {
          var isXSTV2 = isXST && Buffer.compare(version, Buffer.from([0x02, 0x00, 0x00, 0x00])) === 0;
          var treeField = isDecred ? input.tree || Buffer.from([0x00]) : Buffer.alloc(0);
          var data = Buffer.concat([input.prevout, treeField, isXSTV2 ? Buffer.from([0x00]) : _this.createVarint(input.script.length)]);
          return _this.getTrustedInputRaw(data).then(function () {
            ...
            return isDecred ? processWholeScriptBlock(Buffer.concat([input.script, input.sequence])) : isXSTV2 ? processWholeScriptBlock(input.sequence) : processScriptBlocks(input.script, input.sequence);
          });
        }).then(function () {
      ...
      var processOutputs = function processOutputs() {
        return (0, _utils.eachSeries)(outputs, function (output) {
          ...
          _this.createVarint(output.script.length), output.script, Buffer.from(output.postfix || '', 'hex')]);
          return _this.getTrustedInputRaw(data).then(function () {
            // iteration (eachSeries) ended
            // TODO notify progress
    ...
    value: function hashOutputFull(outputScript) {
      ...
      var outputScriptChunks = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

      var offset = 0;
      var p1 = 0x80;
      var isDecred = additionals.includes("decred");

      if (outputScriptChunks) {
          return (0, _utils.asyncWhile)(function () {
              return offset < outputScriptChunks.length;
          }, function () {
              var p1 = offset == (outputScriptChunks.length - 1) ? 0x80 : 0x00;
              var data = outputScriptChunks[offset];
      ...
      key: "hashOutputsPrefix",
      value: function hashOutputsPrefix(prefix) {
          return this.transport.send(0xe0, 0x4a, 0xff, 0x0e, prefix);
      }
  }, {
    key: "signTransaction",
    ...
    value: function createPaymentTransactionNew(inputs, associatedKeysets, changePath, outputScriptHex) {
      ...
      !!expiryHeight && !isDecred ? defaultVersion.writeUInt32LE(sapling ? 0x80000004 : 0x80000003, 0) : isXST ? defaultVersion.writeUInt32LE(2, 0) : defaultVersion.writeUInt32LE(options.version || 1, 0); // Default version to 2 for XST not to have timestamp
      var trustedInputs = [];
      var regularOutputs = [];
      var signatures = [];
      var publicKeys = [];
      ...
      var outputsPrefix = options.outputsPrefix ? Buffer.from(options.outputsPrefix, "hex") : false;
      var outputScriptChunks = options.outputScriptChunks || false;
          ...
          return (
            ...
            _this5.startUntrustedHashTransactionInput(true, targetTransaction, trustedInputs, true, !!expiryHeight, additionals).then(function () {
              return (0, _utils.doIf)(!resuming && typeof changePath != "undefined", function () {
                ...
                return _this5.hashOutputFull(outputScript, [], outputScriptChunks);
              });
            })
          );
        });
              ...
              return (0, _utils.doIf)(!useBip143, function () {
                return (0, _utils.doIf)(!resuming && typeof changePath != "undefined", function () {
                  ...
                  return _this5.provideOutputFullChangePath(changePath);
                    ...
                    if (outputsPrefix) {
                        return _this5.hashOutputsPrefix(outputsPrefix).then(function () {
                            return _this5.hashOutputFull(outputScript, additionals, outputScriptChunks);
                        });
                    }
